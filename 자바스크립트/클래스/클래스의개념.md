...
1. class 를 다룰때 constructor 는 실과 바늘이다.
2. constructor 는 인스턴스 객체를 초기화 한다, 말그대로 매개변수가 객체안에 있다.
3. constructor 안에있는 매개변수는 this로 바인딩을 해줘야한다.
4. new 를 사용할 때 변수명의 첫번째는 대문자로 하며 함수 실행 후 점 표기법을 사용한다.

    ex)  new Animal().name
5. get은 혼자 쓸 수 없다. 접근자 프로퍼티 명과 데이터 프로퍼티의 명이 겹치지 않아야 한다. get은 값을 반환, set은 값을 수정 및 변경한다.
6. 스태틱으로 선언한 정적 메서드는 new 없이 클래스명으로만 호출이 가능하다. 그리고 그 클레스명으로 호출했을 때 선언한 값만 반환한다.
